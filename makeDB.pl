...
$sth = $dbh->prepare("CREATE TABLE shard (shardid integer primary key autoincrement, status integer default 0, playerx integer, playery integer, time integer, nextuser integer, inuse integer default 0, flags integer, mapUpdates varchar(256), inventory varchar(256), lastused datetime);");
